...
std::vector<unsigned char> png::decode(const std::vector<unsigned char> &image_binary) {
    ...
    while (idx < image_binary.size()) {
        ...
        if (type == "PLTE")
            palette = png::read_plte(std::vector(image_binary.begin() + idx + 8,
                                                 image_binary.begin() + idx + 8 + length),
                                     header.channels, length);
        if (type == "IDAT")
            decoded_image = png::read_idat(header,
...
std::vector<png::RGB> png::read_plte(const std::vector<unsigned char> &binary, uint8_t channels,
                                     int length) {
        ...
        for (int j = 0; j < header.width * header.channels; j++) {
            ...
            uint8_t b =
                (defiltered_binary.size() >= header.width * header.channels)
                    ? defiltered_binary[defiltered_binary.size() - header.width * header.channels]
                    : 0;  // Pixel on the previous line
            uint8_t c =
                (j >= header.channels && defiltered_binary.size() >= header.width * header.channels)
                    ? defiltered_binary[defiltered_binary.size() - header.width * header.channels -
                                        header.channels]
